...
Family::Family() : GedcomRecord()
{
}

/** Initialize a Family object based on a family* struct in the libgedcom GOM */
Family::Family(family* c_family, const Gedcom& gom) : GedcomRecord()
{
	/** For each reference, first check in gom to see if that reference already exists
	 * in the object tree. If so, set the pointer to point to the already existant object
